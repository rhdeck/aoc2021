const data8 = `bg gcdaeb aebg efabdcg abdce cafdbe fcbdeg bdacg gbd cafgd | daecb dcbae gb eabg
...
const lines8 = data8.split("\n");
const input = lines8.map((line) => {
  const [codelist, digitlist] = line.split("|");
  const codes = codelist
    .split(" ")
    .map((code) => code.trim().split("").sort().join(""))
    .filter(Boolean);
  const digits = digitlist
    .split(" ")
    .map((d) => d.trim().split("").sort().join(""))
    .filter(Boolean);
  return { codes, digits };
});
const lengths = {
...
const digits = input.map(({ codes, digits }) => {
  const decoded = Object.entries(lengths).map(([key, value]) => ({
    key,
    code: codes.find((code) => code.length === value),
  }));
  const four = decoded.find(({ key }) => key === "4")!.code;
  const seven = decoded.find(({ key }) => key === "7")!.code;
  const nine = codes.find(
    (code) =>
      code.length === 6 &&
      code.split("").filter((letter) => four!.split("").includes(letter))
        .length === 4
  );
  decoded.push({ key: "9", code: nine! });
  const zero = codes.find(
    (code) =>
      code.length === 6 &&
      code !== nine &&
      code.split("").filter((letter) => seven!.split("").includes(letter))
        .length === 3
  );
  decoded.push({ key: "0", code: zero! });
  const six = codes.find(
    (code) => code.length === 6 && code !== nine && code !== zero
  );
  decoded.push({ key: "6", code: six! });
  const three = codes.find(
    (code) =>
      code.length === 5 &&
      code.split("").filter((letter) => seven!.split("").includes(letter))
        .length === 3
  );
  decoded.push({ key: "3", code: three! });
  const five = codes.find(
    (code) =>
      code.length === 5 &&
      code.split("").filter((letter) => six!.split("").includes(letter))
        .length === 5
  );
  decoded.push({ key: "5", code: five! });
  const two = codes.find((c) => !decoded.find(({ code }) => c === code));
  decoded.push({ key: "2", code: two! });
  let decodedDigits = digits
    .map((code) => ({
      key: decoded.find((d) => d.code === code)?.key,
      code,
    }))
    .filter(({ key }) => key);
  return decodedDigits.map(({ key }) => key);
});
const output1 = digits.reduce(
  (acc, digits) =>
    acc +
    digits.filter((digit) => ["1", "4", "7", "8"].includes(digit!)).length,
  0
);
console.log("Part 1 answer:", output1);
const output2 = digits.reduce((acc, digits) => {
  return acc + parseInt(digits.join(""));
}, 0);
console.log("Part 2 answer:", output2);
